...
int main()
{
	...
	do
	{
		...
		case 5:
		{
			...
			while (pNode != NULL)
			{
				//ïîëó÷åíèå èç pNode->ingr âñåõ èíãðèäèåíòîâ 
				j = i = k = 0;

				do // îáðàáàòûâàåì ñòðîêó èíãðèäèåíòîâ äàííîãî óçëà è çàïèñûâàåì èíãðèäèåíòû ïî îòäåëüíîñòè 
				// â ìàññèâ line_ingr 
				{
					...
					} while (*(pNode->name + k) != ' ' || *(pNode->name + k) != ',' || *(pNode->name + k) != '\0'); // òóò ïðèêîë 
					*(*(line_ingr + i) + j) = '\0';
					i++;
					count_ingr++;
					j = 0;
				...
				do 
				{
					...
					} while (*(pNode->author + k) != ' ' || *(pNode->author + k) != ',' || *(pNode->author + k) != '\0'); // òóò ïðèêîë 
					*(*(line_ingr + i) + j) = '\0';
					i++;
					count_ingr++;
					j = 0;
				...
				do 
				{
					...
					} while (*(pNode->ganre + k) != ' ' || *(pNode->ganre + k) != ',' || *(pNode->ganre + k) != '\0'); // òóò ïðèêîë 
					*(*(line_ingr + i) + j) = '\0';
					i++;
					count_ingr++;
					j = 0;
				...
				flag = 0;
				//ñ÷èòàåì ñêîëüêî äîëæíî áûòü ñîâïàäåíèé 
